...
final class SetupManager
{
    ...
    public function testConnection(array $body): array
    {
        try {
            $pdo = $this->buildPdo(
                (string) ($body['db_host']     ?? 'localhost'),
                (string) ($body['db_port']     ?? '3306'),
                (string) ($body['db_database'] ?? ''),
                (string) ($body['db_username'] ?? 'root'),
                (string) ($body['db_password'] ?? ''),
                5,
            );

            /** @var array{Value: string}|false */
            $row     = $pdo->query("SHOW VARIABLES LIKE 'version'")->fetch(PDO::FETCH_ASSOC);
    ...
    {
        $siteUrl = rtrim((string) ($body['site_url'] ?? 'http://localhost'), '/');
        $appName = (string) ($body['app_name'] ?? 'DebugPHP');
        ...
        $content .= "DB_HOST="     . ($body['db_host']     ?? 'localhost') . "\n";
        $content .= "DB_PORT="     . ($body['db_port']     ?? '3306')      . "\n";
        $content .= "DB_DATABASE=" . ($body['db_database'] ?? 'debugphp')  . "\n";
        $content .= "DB_USERNAME=" . ($body['db_username'] ?? 'root')      . "\n";
        $content .= "DB_PASSWORD=" . ($body['db_password'] ?? '')          . "\n\n";
        $content .= "# Session Settings\n";
        $content .= "SESSION_LIFETIME_HOURS=" . ($body['session_lifetime'] ?? '24') . "\n";
